...
Manual checks for **2.1.6+** (standard `checkout/onepage`, **Firecheckout**, and Express Pay security). Run on **staging** before production.

**Scope:** Duplicate-order predispatch runs on:
...
**Firecheckout requirement:** Checkout form-key validation must be enabled in Magento admin. FC skips validation on M1 &lt; 1.8 or when admin disables it — Bold wallet `saveOrder` inherits that behavior.

## Prerequisites

- Bold Payment Booster enabled; **Firecheckout** (or standard one-page) with Bold SPI and/or Express Pay / wallet buttons.
...
1. **Session A:** Complete checkout with Bold wallet (Apple Pay / Google Pay) on **Firecheckout** or note EPS id from Network tab: `payment[additional_data][order_id]` on `saveOrder`.
...
4. **Session B:** POST `saveOrder` with:
   - B's `form_key`
   - `payment[method]=bold`
...
- `/checkout/onepage/saveOrder`
- `/firecheckout/index/saveOrder`
- `/firecheckout/onecolumn/saveOrder` (if one-column layout enabled)

**Automated:** [curl/saveorder-eps-hijack.sh](curl/saveorder-eps-hijack.sh) with `SAVE_ORDER_URL` set to the FC path.
...
1. **Session A:** Open Firecheckout; start Apple Pay / Google Pay; complete wallet sheet so `expresspay/createOrder` returns an EPS `order_id`. **Do not** place the Magento order.
...
1. **Session A:** Open checkout (FC or one-page); note `quote_id` in Express Pay Ajax or DB `sales_flat_quote.entity_id`.
...
## C. SPI postMessage spoof (Firecheckout)

**Goal:** Untrusted origins cannot trigger order placement via fake SPI events.

1. Open Firecheckout with Bold SPI selected.
2. In DevTools console, run:

   ```javascript
   window.postMessage({eventType:'EVENT_SPI_TOKENIZED', payload:{success:true, payload:{data:{payment_id:'fake'}}}}, '*');
   ```

**Expected (after SPI hardening):**

- Checkout must **not** call `saveOrder` / place order.
- Only messages from the SPI iframe or EPS origin are accepted.

---

## D. Firecheckout regression smoke

- Bold SPI card checkout places order on Firecheckout.
- Express Pay / wallet create order + place order on **`/firecheckout/index/saveOrder`**.
- Change shipping method → Bold payment method remains visible; SPI re-inits without duplicate instances.
- Wallet address sync uses FC Ajax URLs (`firecheckout/index/saveBilling`, etc.) when on Firecheckout.
...
## E. IndexController cart endpoints
